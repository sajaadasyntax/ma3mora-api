/**
 * Fix Procurement Order PO-000043
 * 
 * This script fixes the order status for PO-000043. The order shows as "RECEIVED"
...
async function main() {
  console.log('🔧 Starting fix for PO-000043...\n');
  ...
  if (!order) {
    console.error('❌ Order PO-000043 not found!');
    process.exit(1);
  }
  ...
  // - If nothing received and no receipts: CREATED
  let correctStatus: 'CREATED' | 'PARTIAL' | 'RECEIVED';
  if (allFullyReceived) {
    correctStatus = 'RECEIVED';
  } else if (!hasAnyReceived && order.receipts.length === 0) {
